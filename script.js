...
class Item {
    ...
    updateGeneralOver(){
        this.stats.generalOver = this.getGeneralOver();
    }
...
class GameState {
    constructor(initialStats, infoStats) {
        this.item = new Item("Default Item", 1, initialStats);
...
class UI {
    ...
    updateStatGrid() {
        ...
        Object.entries(this.gameState.item.stats)
    ...
    updatePuits() {
        this.updateElement('puitsValue', this.gameState.item.stats.puits);
    }

    updateOver() {
        this.updateElement('overValue', this.gameState.item.stats.generalOver);
    }

    updateStatModifications(affectedStat, value) {
        if (!this.gameState.item.stats[affectedStat].modifications) {
            this.gameState.item.stats[affectedStat].modifications = {};
        }
        this.gameState.item.stats[affectedStat].modifications[affectedStat] = (this.gameState.item.stats[affectedStat].modifications[affectedStat] || 0) + value;
    ...
    updateModificationDisplay(stat) {
        ...
        if (modifElement) {
            const modifications = this.gameState.item.stats[stat].modifications;
            let totalModif = 0;
            let modifText = '';
    ...
    clearModificationDisplay() {
        Object.keys(this.gameState.item.stats)
            .filter(stat => this.gameState.item.stats[stat].native)
            .forEach(stat => {
                const modifElement = document.getElementById(`${stat}-modif`);
                if (modifElement) {
    ...
    updateAll() {
        ...
        this.updateitems();
        this.updateOver();
        Object.keys(this.gameState.item.stats).forEach(stat => {
            if (this.gameState.item.stats[stat].native) {
...
class Game {
    ...
    applyRune(stat, runeType) {
        const statInfo = this.gameState.item.stats[stat];
        ...
        this.gameState.item.updateStat(stat, oldValue + appliedValue);
        const actualChange = this.gameState.item.stats[stat].valeur - oldValue;
    
        this.handlePuitsLoss(stat, puitsLoss);
    
        this.gameState.item.addRuneHistory(stat, runeType, outcomeMessage);
        this.ui.updateRuneHistory(stat, runeType, outcomeMessage);
        this.ui.updateStatModifications(stat, actualChange);
        this.gameState.item.updateGeneralOver();
        this.ui.updateAll();
    }
    ...
    handlePuitsLoss(statAffected, puitsLoss) {
        const currentPuits = this.gameState.item.stats.puits;
        const actualPuitsLoss = Math.min(currentPuits, puitsLoss);
        
        this.gameState.item.updatePuits(-actualPuitsLoss);
    ...
    distributeExcessLoss(statAffected, excessLoss) {
        const eligibleStats = Object.keys(this.gameState.item.stats).filter(stat => 
            stat !== 'puits' && 
            stat !== 'generalOver' && 
            stat !== statAffected &&
            this.gameState.item.stats[stat].valeur > 0 &&
            this.gameState.item.stats[stat].native
        );
        ...
        while (excessLoss > 0 && eligibleStats.length > 0) {
            ...
            if (puitCost <= excessLoss) {
                const newValue = Math.max(0, this.gameState.item.stats[selectedStat].valeur - runeValue);
                const actualChange = newValue - this.gameState.item.stats[selectedStat].valeur;
                this.gameState.item.updateStat(selectedStat, newValue);
                excessLoss -= puitCost;
    ...
    calculateWeights(eligibleStats) {
        return eligibleStats.map(stat => {
            const { min, max, valeur } = this.gameState.item.stats[stat];
            const range = max - min;
    ...
    clearGameStateModifications() {
        Object.keys(this.gameState.item.stats).forEach(stat => {
            if (this.gameState.item.stats[stat].native) {
                this.gameState.item.stats[stat].modifications = {};
            }
        });
    }
    ...
    fusionner() {
        ...
        if (!this.gameState.item.stats.hasOwnProperty(stat)) {
    ...
    addNewStat(stat) {
        ...
        this.gameState.item.stats[stat] = {
            min: 0,
            max: Infinity,
            valeur: 0,
            over: 0,
